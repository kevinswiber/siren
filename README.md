...
When missing, the default value is ```text```.  Serialization of these fields will depend on the value of the action's ```type``` attribute. See [`type` under Actions, above](https://github.com/kevinswiber/siren#type-1). Optional.
